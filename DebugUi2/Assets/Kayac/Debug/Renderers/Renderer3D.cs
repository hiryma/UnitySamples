...
{
    public class Renderer3D : RendererBase
    {
            ...
            int division = 16)
        {
			...
			*/
            ...
            Vector3 forwardVector = camera.gameObject.transform.forward;
            ...
            for (int i = 0; i <= division; i++)
            {
                ...
                vertices[i0] = p + normal;
                vertices[i1] = p - normal;
                // 鏃は太さ0に収束
                if (arrowHead && (i == division))
                {
                    vertices[i0] = vertices[i1] = end;
                }
                colors[i0] = Color;
                colors[i1] = Color;
                uv[i0] = whiteUv;
            ...
            if (arrowHead)
            {
                ...
                int i1 = vertexCount + 2;
                // 接線正規化
                tangent *= (width * 8f) / tangent.magnitude;
                Vector3 arrowHeadRoot = end - tangent;
                vertices[i0] = arrowHeadRoot + normal;
                vertices[i1] = arrowHeadRoot - normal;
                for (int i = 0; i < 3; i++)
                {
            ...
            float lineWidth = 1f)
        {
            ...
            int lineCount = ((div0 + 1) * (div1 + 1))
                + ((div1 + 1) * (div2 + 1))
                + ((div2 + 1) * (div0 + 1));
            if (((vertexCount + (lineCount * 4)) > capacity)
                || ((indexCount + (lineCount * 6)) > capacity))
            {
                return false;
            }
            SetTexture(FontTexture);
            ...
            Vector3 p2)
        {
            ...
            uv[vertexCount + 0] = whiteUv;
            uv[vertexCount + 1] = whiteUv;
            uv[vertexCount + 2] = whiteUv;
            AddTriangleIndices(0, 1, 2);
            vertexCount += 3;
            return true;
        }
            ...
            Vector3 v1)
        {
            ...
            // 時計回り
            vertices[vertexCount + 0] = p;
            vertices[vertexCount + 1] = p + v0;
            vertices[vertexCount + 2] = vertices[vertexCount + 1] + v1;
            vertices[vertexCount + 3] = p + v1;
        ...
        {
            if (
            ((vertexCount + (8 * 6)) > capacity)
            || ((indexCount + (12 * 6)) > capacity))
            {
                return false;
            }
            ...
            float lineWidth)
        {
            ...
            vertices[vertexCount + 4] = p + v0n + v1n;
            vertices[vertexCount + 5] = vertices[vertexCount + 1] - v0n + v1n;
            vertices[vertexCount + 6] = vertices[vertexCount + 2] - v0n - v1n;
            vertices[vertexCount + 7] = vertices[vertexCount + 3] + v0n - v1n;
