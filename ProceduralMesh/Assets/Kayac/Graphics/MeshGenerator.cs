...
namespace Kayac
{
	public static class MeshGenerator
	{
			...
			bool looped)
		{
			...
			int[] indices;
			var ret = false;
			if (GenerateWall(out vertices, out normals, out uvs, out indices, positions, height, thickness, looped))
			{
...
//			GenerateSmoothTetrahedron(out verticesOut, out normalsOut, out indicesOut);
			GenerateSmoothOctahedron(out verticesOut, out normalsOut, out indicesOut);
			...
			int subdivision)
		{
			...
			int[] indices;
			var ret = false;
			if (GenerateCylinderSide(out vertices, out normals, out uvs, out indices, height, radius, subdivision))
			{
				FillMesh(mesh, vertices, normals, uvs, indices);
				ret = true;
			}
		...
		// 角で法線共有しているために滑らか。基本再分割してより滑らかな図形を作るための種として使う。
		public static void GenerateSmoothOctahedron(
			out Vector3[] vertices,
			out Vector3[] normals,
			out int[] indices)
		{
