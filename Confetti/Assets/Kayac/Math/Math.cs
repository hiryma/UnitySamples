...
namespace Kayac
{
	static class Math
	{
		...
		public static void ToCartesian(out Vector3 o, float xAngle, float yAngle)
		{
			o.x = Mathf.Sin(xAngle) * Mathf.Cos(yAngle);
			o.y = Mathf.Sin(xAngle) * Mathf.Sin(yAngle);
			o.z = Mathf.Cos(xAngle);
		}
